...
const ProfilePicturesPopUp = ({closePopUp, openPopup, imgsUrl}) => {

    const handlelosePopUp = (e) => {
        if (e.target.id === 'ProfilePicturesPopUp') {
          closePopUp();
        }
      }
    ...
    className='fixed inset-0 bg-black flex justify-center items-center bg-opacity-20 backdrop-blur-sm  overflow-y-auto'>
      ...
      <div
        ...
        <div className="px-4 mt-5 shadow w-auto self-center rounded-lg dark:bg-dark-second">
            ...
            {imgsUrl.map(pic => {
                return <img src={pic} className='flex w-full h-[500px] mt-10 rounded-md p-1' />
            })}
...
export default ProfilePicturesPopUp